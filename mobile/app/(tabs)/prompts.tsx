...
export default function PromptsScreen() {
  ...
  const loadPrompts = async (token: string) => {
    const [todayResponse, historyResponse] = await Promise.all([
      fetch(`${API_BASE_URL}/prompts/today`, { headers: authHeaders(token) }),
      fetch(`${API_BASE_URL}/prompts/history`, { headers: authHeaders(token) }),
    ]);
  ...
  const submitAnswer = async () => {
    ...
    setMessage('');
    try {
      const response = await fetch(`${API_BASE_URL}/prompts/answer`, {
        method: 'POST',
        headers: {
      ...
      setAnswerText('');
      await loadPrompts(session.token);
    } catch (error) {
      ...
      {today ? (
        <>
          <View style={styles.promptCard}>
            {!!today.prompt.category && <Text style={styles.category}>{today.prompt.category}</Text>}
            <Text style={styles.promptText}>{today.prompt.promptText}</Text>
          </View>
          {!today.answered ? (
            <>
              <TextInput
            ...
            <View style={styles.statusCard}>
              ...
              {today.answers?.map((answer, index) => (
                <Text style={styles.answer} key={`${answer.answeredBy.toString()}-${index}`}>
                  {typeof answer.answeredBy === 'object' ? answer.answeredBy.name : 'Answer'}: {answer.answerText}
                </Text>
              ))}
            </View>
          ) : (
        ...
        <EmptyState message="A thoughtful prompt will be ready soon." />
      )}
      {!!message && <Text style={styles.error}>{message}</Text>}
      <Text style={styles.historyTitle}>Prompt History</Text>
      <FlatList
        data={history}
        keyExtractor={(item, index) => `${item.date}-${item.prompt._id}-${index}`}
        contentContainerStyle={styles.historyList}
        ListEmptyComponent={<EmptyState message="Your thoughtful answers will collect here." />}
        renderItem={({ item }) => (
          <View style={styles.historyItem}>
            <Text style={styles.historyDate}>{new Date(item.date).toLocaleDateString()}</Text>
            <Text style={styles.historyPrompt}>{item.prompt.promptText}</Text>
...
const styles = StyleSheet.create({
  ...
  promptCard: { backgroundColor: '#d9c5f4', borderRadius: 18, padding: 20, marginBottom: 16, shadowColor: '#2e2050', shadowOpacity: 0.08, shadowRadius: 10, elevation: 2 },
  category: { color: '#62409d', fontSize: 13, fontWeight: '700', textTransform: 'uppercase', marginBottom: 8 },
  promptText: { color: '#302443', fontSize: 21, lineHeight: 29, fontWeight: '700' },
  ...
  historyPrompt: { color: '#302443', fontSize: 17, lineHeight: 23, fontWeight: '700' },
